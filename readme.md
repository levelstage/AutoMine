...
**책 <밑바닥부터 시작하는 딥러닝>을 읽고, "지뢰찾기에 적용 가능할거같은데?"하는 생각으로 시작한 프로젝트입니다.**
...
지뢰찾기 게임 본체는 **C# (MonoGame)**으로 개발했고, AI 연산은 **Python**에서 돌아갑니다. 두 개의 서로 다른 프로그램이 실시간으로 통신하며 작동하는 구조를 만들었습니다.
...
> **⚠️ 주의:** 현재 버전은 소스 코드 내에 파일 경로가 **절대 경로**로 하드코딩되어 있어, 다른 환경에서 실행하기 매우 번거롭습니다. **실행을 권장드리지 않습니다.**
